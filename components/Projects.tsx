      ...
      <div className="max-w-[1200px] mx-auto" ref={ref}>
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12">
            <div>
              <h2 className="text-4xl md:text-5xl font-display font-bold text-text-primary mb-3">
                Professional Work
              </h2>
          ...
          {professionalProjectsData.map((project, i) => (
            <Link key={i} href={project.internalLink}>
              <motion.article
                ...
                className="card-glow group relative bg-bg-secondary/40 border border-border rounded-3xl overflow-hidden hover:bg-bg-secondary/60 hover:border-border-hover transition-all duration-500 flex flex-col h-full hover:-translate-y-2 hover:shadow-xl hover:shadow-accent/5"
              >
                {/* Gradient accent bar */}
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-semibold tracking-wide uppercase px-3 py-1 rounded-full bg-accent-subtle text-accent font-mono">
                      {project.company}
                    </span>
                  ...
                  <div className="border-t border-border pt-4 flex items-center justify-between">
                    <div>
                      <span className="text-lg font-display font-bold text-text-primary group-hover:text-accent transition-colors">
                        {project.impact}
                      </span>
                    </div>
                    <div className="w-10 h-10 rounded-full border border-border flex items-center justify-center text-text-tertiary group-hover:bg-accent group-hover:border-accent group-hover:text-white transition-all group-hover:scale-110">
        ...
        {/* Side Projects — Horizontal Scroll Carousel */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
        ...
        >
          <div className="flex items-end justify-between gap-4 mb-8">
            ...
            <span className="hidden md:flex items-center gap-1 text-xs text-text-tertiary font-mono whitespace-nowrap">
              Scroll &rarr;
            </span>
          </div>
        </motion.div>
            ...
            {sideProjectsData.map((project, i) => {
              ...
              const cardContent = (
                <div className="w-[340px] md:w-[400px]">
                  ...
                  <div className="mb-4">
                    <p className="text-xs text-text-tertiary font-mono uppercase tracking-wide mb-1">Built with</p>
              ...
              return hasInternalLink ? (
                <Link key={i} href={(project as { internalLink: string }).internalLink}>
                  <motion.div
                    ...
                    className="card-glow group bg-bg-secondary/40 border border-border rounded-3xl p-6 hover:border-border-hover hover:bg-bg-secondary/60 transition-all duration-500 cursor-pointer h-full hover:-translate-y-1"
                  >
                    {cardContent}
                  </motion.div>
                </Link>
                ...
                <motion.div
                  ...
                  animate={isInView ? { opacity: 1, x: 0 } : {}}
                  transition={{ delay: 0.4 + i * 0.1, duration: 0.5 }}
                  className="card-glow group bg-bg-secondary/40 border border-border rounded-3xl p-6 hover:border-border-hover hover:bg-bg-secondary/60 transition-all duration-500 h-full hover:-translate-y-1"
                >
                  {cardContent}
                </motion.div>
              );
